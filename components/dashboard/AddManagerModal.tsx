...
export function AddManagerModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await brokerageService.createManager({
      ...
      alert(
        `Manager created successfully!\n\nEmail: ${response.user.email}\nUser ID: ${response.user.user_id}\n\nAn invitation email has been sent to the manager.`,
      );
      ...
      setError(err.message || "Failed to create manager. Please try again.");
    } finally {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6 flex gap-3">
            ...
            >
              {isSubmitting ? "Creating..." : "Add Manager"}
            </button>
          </div>
        </form>
      </div>
