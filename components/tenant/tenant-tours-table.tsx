...
export function TenantToursTable() {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        const data = await tenantService.getMyTours();
        setTours(data.tours);
      } catch (error) {
  ...
  return (
    <div className="bg-card rounded-lg p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-2">My Tours</h1>
        <p className="text-gray-600">View and manage your property tours</p>
      </div>
      ...
      {tours.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400">
            Book a property tour to see it here
          </p>
        </div>
      ) : (
        <>
