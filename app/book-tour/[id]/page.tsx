...
export default function BookTourPage() {
  ...
  // Fetch bids for this listing (only if authenticated)
  useEffect(() => {
    if (!isAuthenticated) {
      // Skip fetching bids if not authenticated
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        if (isAuthenticated) {
          console.log(
            "🔐 Using authenticated tenant endpoint for property details",
          );
          data = await tenantService.getListingDetails(id);
        } else {
          console.log("🌐 Using public listing endpoint for property details");
          data = await listingService.getListingDetails(id);
        }
